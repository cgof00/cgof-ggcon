...
var onRequest8 = /* @__PURE__ */ __name(async (context) => {
  ...
  __name(callRpc, "callRpc");
  try {
    console.log("\u{1F504} Sincronizando incrementalmente (compara\xE7\xE3o total, anos 2023-2026)...");
    const result = await callRpc("sync_emendas_formalizacao");
    console.log("\u2705 Sincroniza\xE7\xE3o sucesso:", result);
    ...
    try {
      ...
      if (deleteResp.ok) {
        emendasCleaned = true;
        console.log("\u{1F9F9} Tabela emendas limpa ap\xF3s sincroniza\xE7\xE3o (economia de espa\xE7o)");
      } else {
        console.warn("\u26A0\uFE0F N\xE3o foi poss\xEDvel limpar tabela emendas:", await deleteResp.text());
      }
    } catch (cleanErr) {
    ...
    return new Response(JSON.stringify({
      ...
      result: {
        ...
        message: result?.message || "Sincroniza\xE7\xE3o conclu\xEDda"
      }
    }), {
      status: 200,
      headers: { "Content-Type": "application/json" }
...
var onRequest11 = /* @__PURE__ */ __name(async (context) => {
  ...
  if (request.method === "POST" && url.pathname === "/api/auth/login") {
    try {
      ...
      const hashCalc = await hashPassword4(senha);
...
var onRequest25 = /* @__PURE__ */ __name(async (context) => {
  ...
  const decoded = verifyToken5(authHeader.slice(7));
  if (!decoded) {
    return new Response(JSON.stringify({ error: "Token inv\xE1lido ou expirado" }), {
      status: 401,
      headers: { "Content-Type": "application/json" }
...
// ../.wrangler/tmp/pages-BPhdUg/functionsRoutes-0.2266638970349063.mjs
var routes = [
  {
