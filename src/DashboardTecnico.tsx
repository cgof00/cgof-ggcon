 ...
 *  Delega ao utilitário compartilhado; retorna '(não informado)' quando sem dados. */
function getAreaEstagio(r: FormalizacaoRow): string {
  return deriveAreaEstagio(r) || '(não informado)';
}

/** Paleta de cor por área para o demonstrativo — suporta nomes com prefixo numérico oficial */
function getAreaPalette(area: string): { bg: string; dot: string; bar: string; border: string; num: string } {
  const a = area.toLowerCase();
  // ── Áreas oficiais numeradas (sem papel) ──────────────────────────────────
  if (a === '1 - beneficiário' || a === 'beneficiário')
    return { bg: 'bg-blue-50',    dot: 'bg-blue-500',    bar: 'bg-blue-500',    border: 'border-blue-200',    num: 'text-blue-700' };
  if (a === '2 - drs' || a === 'drs' || a === 'drs ou fundo a fundo')
    return { bg: 'bg-teal-50',    dot: 'bg-teal-500',    bar: 'bg-teal-500',    border: 'border-teal-200',    num: 'text-teal-700' };
  if (a === '3 - crs' || a === 'crs')
    return { bg: 'bg-cyan-50',    dot: 'bg-cyan-500',    bar: 'bg-cyan-500',    border: 'border-cyan-200',    num: 'text-cyan-700' };
  if (a === '4 - cgof - ggcon' || a === 'ggcon cgof')
    return { bg: 'bg-violet-50',  dot: 'bg-violet-500',  bar: 'bg-violet-500',  border: 'border-violet-200',  num: 'text-violet-700' };
  if (a === '4.1 - cgof - orçamento' || a === 'orçamento cgof')
    return { bg: 'bg-yellow-50',  dot: 'bg-yellow-500',  bar: 'bg-yellow-500',  border: 'border-yellow-200',  num: 'text-yellow-700' };
  if (a === '4.2 - cgof - financeiro' || a === 'financeiro cgof')
    return { bg: 'bg-green-50',   dot: 'bg-green-500',   bar: 'bg-green-500',   border: 'border-green-200',   num: 'text-green-700' };
  if (a === '4.3 - cgof - coordenador' || a === 'coordenador cgof')
    return { bg: 'bg-purple-50',  dot: 'bg-purple-500',  bar: 'bg-purple-500',  border: 'border-purple-200',  num: 'text-purple-700' };
  if (a === '5 - chefia de gabinete' || a === 'chefia de gabinete')
    return { bg: 'bg-amber-50',   dot: 'bg-amber-500',   bar: 'bg-amber-500',   border: 'border-amber-200',   num: 'text-amber-700' };
  if (a === '6 - secretário' || a === 'secretário')
    return { bg: 'bg-pink-50',    dot: 'bg-pink-500',    bar: 'bg-pink-500',    border: 'border-pink-200',    num: 'text-pink-700' };
  if (a === '7 - em assinatura' || a === 'aguardando assinaturas')
    return { bg: 'bg-sky-50',     dot: 'bg-sky-500',     bar: 'bg-sky-500',     border: 'border-sky-200',     num: 'text-sky-700' };
  if (a === '8 - pagamento' || a === 'repasse fundo a fundo' || a === 'repasse próprio beneficiário')
    return { bg: 'bg-indigo-50',  dot: 'bg-indigo-400',  bar: 'bg-indigo-400',  border: 'border-indigo-200',  num: 'text-indigo-700' };
  if (a === '9 - concluído' || a === 'concluída')
    return { bg: 'bg-emerald-50', dot: 'bg-emerald-500', bar: 'bg-emerald-500', border: 'border-emerald-200', num: 'text-emerald-700' };
  if (a === '9.1 - cancelada' || a === 'cancelada / impedida' || a.includes('cancelad'))
    return { bg: 'bg-red-50',     dot: 'bg-red-500',     bar: 'bg-red-500',     border: 'border-red-200',     num: 'text-red-700' };
  if (a === '9.2 - impedida' || a.includes('impedid'))
    return { bg: 'bg-orange-50',  dot: 'bg-orange-500',  bar: 'bg-orange-500',  border: 'border-orange-200',  num: 'text-orange-700' };
  // ── Áreas sem código oficial ──────────────────────────────────────────────
  if (a === 'saúde animal')              return { bg: 'bg-lime-50',   dot: 'bg-lime-500',   bar: 'bg-lime-500',   border: 'border-lime-200',   num: 'text-lime-700' };
  if (a === 'casa civil')                return { bg: 'bg-rose-50',   dot: 'bg-rose-500',   bar: 'bg-rose-500',   border: 'border-rose-200',   num: 'text-rose-700' };
  if (a === 'secretaria de governo')     return { bg: 'bg-rose-50',   dot: 'bg-rose-400',   bar: 'bg-rose-400',   border: 'border-rose-200',   num: 'text-rose-600' };
  if (a === 'impedimento eleitoral')     return { bg: 'bg-slate-50',  dot: 'bg-slate-400',  bar: 'bg-slate-400',  border: 'border-slate-200',  num: 'text-slate-600' };
        ...
        <>
          ...
          {(() => {
            ...
            const areas = Array.from(areaMap.entries())
              ...
              .sort((a, b) => {
                const order = (lbl: string) => { const m = lbl.match(/^(\d+(?:\.\d+)?)\s*-/); return m ? parseFloat(m[1]) : 98; };
                return order(a.label) - order(b.label) || b.count - a.count;
              });
