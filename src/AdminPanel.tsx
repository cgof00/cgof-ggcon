...
export function AdminPanel() {
  ...
  const displayData = useMemo(() => {
    ...
    const situacoes = [
      ...
      'EM ANÁLISE ORÇAMENTÁRIA CGOF – FUNDO A FUNDO',
      'PARECER COORDENADOR CGOF – FUNDO A FUNDO',
      'APROVAÇÃO - CHEFIA DE GABINETE – FUNDO A FUNDO',
      'AGUARDANDO APROVAÇÃO DO SECRETARIO DE ESTADO DA SAÚDE – FUNDO A FUNDO',
      'AGUARDANDO RESOLUÇÃO PARA EMISSÃO RESOLUÇÃO PARA REPASSE FUNDO A FUNDO - DOE'
    ];
  ...
  const carregarDashboard = async () => {
    try {
      ...
      // Deduplica por número de demanda: emendas agregadoras têm múltiplas linhas
      // com o mesmo demandas_formalizacao — contam como UMA demanda.
      // Mantém o registro com data_liberacao mais recente (desempate: maior id).
      const keepRow = (a: any, b: any): any => {
        const da = a.data_liberacao ? new Date(String(a.data_liberacao)).getTime() : 0;
        const db = b.data_liberacao ? new Date(String(b.data_liberacao)).getTime() : 0;
      ...
      for (const r of allData) {
        ...
        deduped.set(key, existing ? keepRow(existing, r) : r);
      }
      const dedupedData = Array.from(deduped.values());
      setRawFormalizacoes(dedupedData);
      setTotalRecords(dedupedData.length);
    } catch (err) {
