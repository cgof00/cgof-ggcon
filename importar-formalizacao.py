#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
IMPORTAR FORMALIZAÇÃO DIRETAMENTE NO SUPABASE (VERSÃO RÁPIDA)
=============================================================
Usa batches de 500 registros + 8 threads paralelas.
Limpa a tabela via SQL direto (TRUNCATE) em vez de deletar row-by-row.

USO:
  python importar-formalizacao.py

REQUISITOS:
  pip install requests python-dotenv
"""

import csv
import os
import re
import sys
import json
import time
from concurrent.futures import ThreadPoolExecutor, as_completed

try:
    import requests
    import urllib3
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
except ImportError:
    print("❌ Módulo 'requests' não encontrado.")
    print("   Execute: pip install requests")
    sys.exit(1)

try:
    from dotenv import load_dotenv
except ImportError:
    print("❌ Módulo 'python-dotenv' não encontrado.")
    print("   Execute: pip install python-dotenv")
    sys.exit(1)

# ==============================================================
# CONFIGURAÇÃO
# ==============================================================

# Caminho do CSV de formalização
CSV_FILE = r'C:\Users\afpereira\Downloads\geral.csv'

# Modo de importação: 'replace' (apaga tudo e reimporta) ou 'append' (adiciona)
MODO = 'replace'

# Tamanho dos batches para inserção (500 é o sweet spot para Supabase REST)
BATCH_SIZE = 500

# Número de threads paralelas para inserção
PARALLEL_WORKERS = 8

# ==============================================================
# MAPEAMENTO DE CABEÇALHOS CSV → COLUNAS DO BANCO
# ==============================================================
HEADER_MAP = {
    "Seq": "seq",
    "Ano": "ano",
    "Parlamentar": "parlamentar",
    "Partido": "partido",
    "Emenda": "emenda",
    "Emendas Agregadoras": "emendas_agregadoras",
    "Demanda": "demanda",
    "DEMANDAS FORMALIZAÇÃO": "demandas_formalizacao",
    "DEMANDAS FORMALIZACAO": "demandas_formalizacao",
    "N° de Convênio": "numero_convenio",
    "N° de Convenio": "numero_convenio",
    "N\u00b0 de Conv\u00eanio": "numero_convenio",
    "Classificação Emenda/Demanda": "classificacao_emenda_demanda",
    "Classificacao Emenda/Demanda": "classificacao_emenda_demanda",
    "Tipo de Formalização": "tipo_formalizacao",
    "Tipo de Formalizacao": "tipo_formalizacao",
    "Regional": "regional",
    "Município": "municipio",
    "Municipio": "municipio",
    "Conveniado": "conveniado",
    "Objeto": "objeto",
    "Portfólio": "portfolio",
    "Portfolio": "portfolio",
    "Valor": "valor",
    "Posição Anterior": "posicao_anterior",
    "Posicao Anterior": "posicao_anterior",
    "Situação Demandas - SemPapel": "situacao_demandas_sempapel",
    "Situacao Demandas - SemPapel": "situacao_demandas_sempapel",
    "Área - estágio": "area_estagio",
    "Area - estagio": "area_estagio",
    "Recurso": "recurso",
    "Tecnico": "tecnico",
    "Data da Liberação": "data_liberacao",
    "Data da Liberacao": "data_liberacao",
    "Área - Estágio Situação da Demanda": "area_estagio_situacao_demanda",
    "Area - Estagio Situacao da Demanda": "area_estagio_situacao_demanda",
    "Situação - Análise Demanda": "situacao_analise_demanda",
    "Situacao - Analise Demanda": "situacao_analise_demanda",
    "Data - Análise Demanda": "data_analise_demanda",
    "Data - Analise Demanda": "data_analise_demanda",
    "Motivo do Retorno da Diligência": "motivo_retorno_diligencia",
    "Motivo do Retorno da Diligencia": "motivo_retorno_diligencia",
    "Data do Retorno da Diligência": "data_retorno_diligencia",
    "Data do Retorno da Diligencia": "data_retorno_diligencia",
    "Conferencista": "conferencista",
    "Data recebimento demanda": "data_recebimento_demanda",
    "Data do Retorno": "data_retorno",
    "Observação - Motivo do Retorno": "observacao_motivo_retorno",
    "Observacao - Motivo do Retorno": "observacao_motivo_retorno",
    "Data liberação da Assinatura - Conferencista": "data_liberacao_assinatura_conferencista",
    "Data liberacao da Assinatura - Conferencista": "data_liberacao_assinatura_conferencista",
    "Data liberação de Assinatura": "data_liberacao_assinatura",
    "Data liberacao de Assinatura": "data_liberacao_assinatura",
    "Falta assinatura": "falta_assinatura",
    "Assinatura": "assinatura",
    "Publicação": "publicacao",
    "Publicacao": "publicacao",
    "Vigência": "vigencia",
    "Vigencia": "vigencia",
    "Encaminhado em": "encaminhado_em",
    "Concluída em": "concluida_em",
    "Concluida em": "concluida_em",
}

# Colunas que são do tipo DATE no banco (precisam de conversão DD/MM/YYYY → YYYY-MM-DD)
DATE_COLUMNS = {
    "data_liberacao", "data_analise_demanda", "data_retorno_diligencia",
    "data_recebimento_demanda", "data_retorno",
    "data_liberacao_assinatura_conferencista", "data_liberacao_assinatura",
    "encaminhado_em", "concluida_em"
}

# Colunas que são TEXT no banco mas que podem conter datas (manter como texto)
TEXT_DATE_COLUMNS = {
    "publicacao", "vigencia", "assinatura"
}

# ==============================================================
# FUNÇÕES AUXILIARES
# ==============================================================

def convert_date(val):
    """Converte DD/MM/YYYY → YYYY-MM-DD. Retorna None se inválido."""
    if not val or not val.strip():
        return None
    val = val.strip()
    # Tentar DD/MM/YYYY
    m = re.match(r'^(\d{1,2})/(\d{1,2})/(\d{4})$', val)
    if m:
        dia, mes, ano = m.groups()
        dia = dia.zfill(2)
        mes = mes.zfill(2)
        # Validar data
        if 1 <= int(mes) <= 12 and 1 <= int(dia) <= 31:
            return f"{ano}-{mes}-{dia}"
    # Tentar YYYY-MM-DD (já está no formato certo)
    m2 = re.match(r'^(\d{4})-(\d{2})-(\d{2})$', val)
    if m2:
        return val
    # Tentar DD/MM/YY
    m3 = re.match(r'^(\d{1,2})/(\d{1,2})/(\d{2})$', val)
    if m3:
        dia, mes, ano_curto = m3.groups()
        ano_full = '20' + ano_curto if int(ano_curto) < 50 else '19' + ano_curto
        dia = dia.zfill(2)
        mes = mes.zfill(2)
        if 1 <= int(mes) <= 12 and 1 <= int(dia) <= 31:
            return f"{ano_full}-{mes}-{dia}"
    # Não conseguiu converter - retorna None para não quebrar o INSERT
    return None


def convert_valor(val):
    """Converte valor brasileiro (1.234.567,89) → float. Retorna None se inválido."""
    if not val or not val.strip():
        return None
    val = val.strip()
    # Remover R$, espaços, etc
    val = re.sub(r'[R$\s]', '', val)
    if not val:
        return None
    # Formato brasileiro: 1.234.567,89 → remover pontos de milhar, trocar vírgula por ponto
    if ',' in val:
        val = val.replace('.', '').replace(',', '.')
    elif val.count('.') > 1:
        # Múltiplos pontos = separadores de milhar sem vírgula decimal
        val = val.replace('.', '')
    try:
        return float(val)
    except (ValueError, OverflowError):
        return None


def map_row(row, header_indices):
    """Mapeia uma linha do CSV para um dict com nomes de colunas do banco."""
    record = {}
    for csv_col, (idx, db_col) in header_indices.items():
        if idx >= len(row):
            continue
        val = row[idx].strip() if row[idx] else ''
        if not val:
            continue
        if db_col == 'valor':
            converted = convert_valor(val)
            if converted is not None:
                record[db_col] = converted
        elif db_col in DATE_COLUMNS:
            converted = convert_date(val)
            if converted is not None:
                record[db_col] = converted
        else:
            record[db_col] = val
    return record


def load_env():
    """Carrega variáveis do .env.local"""
    env_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '.env.local')
    if os.path.exists(env_path):
        load_dotenv(env_path)
        print(f"✅ .env.local carregado de: {env_path}")
    else:
        print(f"⚠️  .env.local não encontrado em: {env_path}")
        print("   Tentando variáveis de ambiente do sistema...")
    
    url = os.environ.get('SUPABASE_URL')
    key = os.environ.get('SUPABASE_SERVICE_ROLE_KEY')
    
    if not url or not key:
        print("❌ SUPABASE_URL e/ou SUPABASE_SERVICE_ROLE_KEY não configurados!")
        print("   Configure no .env.local ou nas variáveis de ambiente do sistema.")
        sys.exit(1)
    
    return url, key


# ==============================================================
# MAIN
# ==============================================================

def main():
    print("=" * 70)
    print("  IMPORTAR FORMALIZAÇÃO → SUPABASE (IMPORT DIRETO VIA API)")
    print("=" * 70)
    print()
    
    # Verificar arquivo CSV
    if not os.path.exists(CSV_FILE):
        print(f"❌ Arquivo CSV não encontrado: {CSV_FILE}")
        print("   Edite a variável CSV_FILE no início deste script.")
        sys.exit(1)
    
    file_size = os.path.getsize(CSV_FILE) / (1024 * 1024)
    print(f"📄 Arquivo: {CSV_FILE}")
    print(f"📦 Tamanho: {file_size:.1f} MB")
    print()
    
    # Carregar configuração
    supabase_url, supabase_key = load_env()
    rest_url = f"{supabase_url}/rest/v1"
    headers = {
        "apikey": supabase_key,
        "Authorization": f"Bearer {supabase_key}",
        "Content-Type": "application/json",
        "Prefer": "return=minimal"
    }
    
    print(f"🔗 Supabase URL: {supabase_url}")
    print()
    
    # ============================================================
    # PASSO 1: Ler CSV
    # ============================================================
    print("📖 PASSO 1: Lendo CSV...")
    
    records = []
    skipped_rows = []
    date_conversion_failures = []
    
    # Usar utf-8-sig para remover BOM automaticamente (\ufeff)
    for encoding in ['utf-8-sig', 'utf-8', 'latin-1', 'cp1252']:
        try:
            with open(CSV_FILE, 'r', encoding=encoding) as f:
                first_line = f.readline()
                if first_line:
                    print(f"   Encoding: {encoding} ✓")
                    break
        except (UnicodeDecodeError, UnicodeError):
            continue
    else:
        encoding = 'utf-8-sig'
    
    with open(CSV_FILE, 'r', encoding=encoding) as f:
        # Detectar delimitador
        first_line = f.readline()
        f.seek(0)
        delimiter = ';' if first_line.count(';') > first_line.count(',') else ','
        print(f"   Delimitador: '{delimiter}'")
        
        reader = csv.reader(f, delimiter=delimiter)
        
        # Processar header
        header_row = next(reader)
        header_row = [h.strip() for h in header_row]
        
        # Mapear cabeçalhos → índices e nomes do banco
        header_indices = {}
        unmapped = []
        for i, col_name in enumerate(header_row):
            if col_name in HEADER_MAP:
                db_col = HEADER_MAP[col_name]
                header_indices[col_name] = (i, db_col)
            elif col_name.lower() in ('', '5ultimos'):
                continue  # Colunas ignoradas
            else:
                # Tentar match case-insensitive
                found = False
                for csv_key, db_key in HEADER_MAP.items():
                    if csv_key.lower().replace(' ', '') == col_name.lower().replace(' ', ''):
                        header_indices[col_name] = (i, db_key)
                        found = True
                        break
                if not found:
                    unmapped.append(col_name)
        
        print(f"   Colunas mapeadas: {len(header_indices)}")
        if unmapped:
            print(f"   ⚠️  Colunas não mapeadas (ignoradas): {unmapped}")
        
        # Processar linhas de dados
        row_num = 1  # header é linha 1
        for row in reader:
            row_num += 1
            try:
                record = map_row(row, header_indices)
                if record:  # Pelo menos um campo preenchido
                    records.append(record)
                else:
                    skipped_rows.append((row_num, "Linha completamente vazia"))
            except Exception as e:
                skipped_rows.append((row_num, str(e)))
    
    total_csv = row_num - 1  # Total de linhas de dados (excluindo header)
    print(f"\n   📊 RESULTADO DA LEITURA:")
    print(f"      Total de linhas no CSV: {total_csv}")
    print(f"      Registros válidos:      {len(records)}")
    print(f"      Linhas ignoradas:       {len(skipped_rows)}")
    
    if skipped_rows and len(skipped_rows) <= 20:
        print(f"\n   Linhas ignoradas:")
        for row_num, reason in skipped_rows:
            print(f"      Linha {row_num}: {reason}")
    elif skipped_rows:
        print(f"\n   Primeiras 10 linhas ignoradas:")
        for row_num, reason in skipped_rows[:10]:
            print(f"      Linha {row_num}: {reason}")
    
    if len(records) == 0:
        print("\n❌ Nenhum registro para importar!")
        sys.exit(1)
    
    print()
    
    # ============================================================
    # PASSO 2: Limpar tabela (modo replace) — via SQL TRUNCATE (instantâneo)
    # ============================================================
    if MODO == 'replace':
        print("🗑️  PASSO 2: Limpando tabela formalizacao via TRUNCATE...")
        
        # Usar endpoint SQL do Supabase (rpc) para TRUNCATE — muito mais rápido que deletar row-by-row
        # Primeiro, criar uma função RPC temporária para truncate
        sql_headers = {
            "apikey": supabase_key,
            "Authorization": f"Bearer {supabase_key}",
            "Content-Type": "application/json",
        }
        
        # Tentar via RPC chamando uma função que faz truncate
        # Primeiro verificar se existe
        rpc_resp = requests.post(
            f"{rest_url}/rpc/truncate_formalizacao",
            headers=sql_headers,
            json={},
            verify=False,
        )
        
        if rpc_resp.status_code in (200, 204):
            print("   ✅ Tabela limpa via TRUNCATE (RPC)")
        else:
            # Fallback: deletar em batches grandes e paralelos
            print("   ⚠️  RPC truncate não disponível, deletando em batches...")
            print("   💡 Para acelerar, execute no SQL Editor do Supabase:")
            print("      CREATE OR REPLACE FUNCTION truncate_formalizacao() RETURNS void AS $$")
            print("      BEGIN TRUNCATE TABLE formalizacao RESTART IDENTITY CASCADE; END; $$ LANGUAGE plpgsql SECURITY DEFINER;")
            print("      GRANT EXECUTE ON FUNCTION truncate_formalizacao() TO service_role;")
            print()
            
            deleted_total = 0
            while True:
                fetch_resp = requests.get(
                    f"{rest_url}/formalizacao?select=id&limit=5000",
                    headers=headers,
                    verify=False,
                )
                if fetch_resp.status_code != 200:
                    print(f"   ❌ Erro ao buscar IDs: {fetch_resp.status_code}")
                    break
                
                ids = [r['id'] for r in fetch_resp.json()]
                if not ids:
                    break
                
                # Deletar em sub-batches paralelos de 1000
                def delete_chunk(chunk_ids):
                    id_list = ','.join(str(i) for i in chunk_ids)
                    return requests.delete(
                        f"{rest_url}/formalizacao?id=in.({id_list})",
                        headers=headers,
                        verify=False,
                    )
                
                chunk_size = 1000
                chunks = [ids[j:j+chunk_size] for j in range(0, len(ids), chunk_size)]
                with ThreadPoolExecutor(max_workers=4) as executor:
                    list(executor.map(delete_chunk, chunks))
                
                deleted_total += len(ids)
                print(f"   🗑️  {deleted_total} registros deletados...", end='\r')
            
            print(f"   ✅ {deleted_total} registros removidos" + " " * 30)
        
        print()
    
    # ============================================================
    # PASSO 3: Inserir registros em batches PARALELOS
    # ============================================================
    print(f"📥 PASSO 3: Inserindo {len(records)} registros ({BATCH_SIZE}/batch, {PARALLEL_WORKERS} threads)...")
    
    total_inserted = 0
    total_errors = 0
    error_details = []
    start_time = time.time()
    
    # Preparar batches
    batches = []
    for i in range(0, len(records), BATCH_SIZE):
        batch = records[i:i + BATCH_SIZE]
        batches.append((i, batch))
    
    total_batches = len(batches)
    completed_batches = 0
    
    def insert_batch(args):
        """Insere um batch via REST API. Retorna (inserted, errors)."""
        offset, batch = args
        try:
            resp = requests.post(
                f"{rest_url}/formalizacao",
                headers=headers,
                json=batch,
                timeout=60,
                verify=False,
            )
            if resp.status_code in (200, 201):
                return len(batch), 0, []
            else:
                # Batch falhou — tentar inserir individualmente
                ok = 0
                errs = 0
                err_list = []
                for j, record in enumerate(batch):
                    try:
                        single_resp = requests.post(
                            f"{rest_url}/formalizacao",
                            headers=headers,
                            json=[record],
                            timeout=15,
                            verify=False,
                        )
                        if single_resp.status_code in (200, 201):
                            ok += 1
                        else:
                            errs += 1
                            if len(err_list) < 3:
                                err_list.append(f"Linha {offset + j + 2}: {single_resp.text[:150]}")
                    except Exception as e:
                        errs += 1
                        if len(err_list) < 3:
                            err_list.append(f"Linha {offset + j + 2}: {str(e)[:100]}")
                return ok, errs, err_list
        except Exception as e:
            return 0, len(batch), [f"Batch offset {offset}: {str(e)[:150]}"]
    
    # Executar em paralelo
    with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as executor:
        futures = {executor.submit(insert_batch, b): b for b in batches}
        
        for future in as_completed(futures):
            inserted, errors, errs = future.result()
            total_inserted += inserted
            total_errors += errors
            error_details.extend(errs)
            completed_batches += 1
            
            pct = (completed_batches / total_batches) * 100
            elapsed = time.time() - start_time
            rate = total_inserted / elapsed if elapsed > 0 else 0
            print(f"   [{pct:5.1f}%] {completed_batches}/{total_batches} batches | {total_inserted} inseridos ({rate:.0f} reg/s)", end='\r')
    
    elapsed = time.time() - start_time
    print()
    print()
    
    # ============================================================
    # RESULTADO FINAL
    # ============================================================
    print("=" * 70)
    print("  RESULTADO FINAL")
    print("=" * 70)
    print(f"   📄 Linhas no CSV:       {total_csv}")
    print(f"   📖 Registros lidos:     {len(records)}")
    print(f"   ✅ Inseridos no banco:  {total_inserted}")
    print(f"   ❌ Erros:               {total_errors}")
    print(f"   ⏱️  Tempo:              {elapsed:.1f}s")
    print()
    
    if total_errors > 0 and error_details:
        print("   Primeiros erros:")
        for err in error_details[:5]:
            print(f"     {err}")
        print()
    
    if total_inserted == len(records) and len(records) == total_csv:
        print("   🎉 SUCESSO TOTAL! Todos os registros do CSV foram importados!")
    elif total_inserted == len(records):
        print(f"   ✅ Todos os {len(records)} registros válidos foram importados.")
        if len(skipped_rows) > 0:
            print(f"   ⚠️  {len(skipped_rows)} linhas foram ignoradas (vazias ou inválidas).")
    else:
        print(f"   ⚠️  {total_errors} registros não puderam ser inseridos.")
        print(f"      Verifique os erros acima.")
    
    # Verificar contagem no banco
    print()
    print("   📊 Verificando contagem no banco...")
    count_resp = requests.get(
        f"{rest_url}/formalizacao?select=id",
        headers={**headers, "Prefer": "count=exact", "Range": "0-0"},
        verify=False,
    )
    if count_resp.status_code in (200, 206):
        content_range = count_resp.headers.get('Content-Range', '')
        if '/' in content_range:
            db_count = content_range.split('/')[-1]
            print(f"   Total no banco agora: {db_count}")
    
    print("=" * 70)


if __name__ == '__main__':
    main()
