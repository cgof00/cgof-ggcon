...
const app = express();
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      if (supabase) {
        ...
        const token = generateToken(data.id, data.email, data.role, data.nome);
  ...
  app.post("/api/admin/usuarios", authMiddleware, async (req: any, res) => {
    try {
      ...
      console.log('✅ 6️⃣ Senha gerada/fornecida');
  ...
  app.post("/api/emendas/import-report", authMiddleware, async (req: any, res) => {
    try {
      ...
      if (decoded.role !== 'admin') return res.status(403).json({ error: 'Apenas administradores podem importar relatórios' });
      ...
      return res.status(500).json({ error: "Erro ao importar relatório", details: error.message });
    }
  });
  ...
  app.get("/api/formalizacao/:id", authMiddleware, async (req: any, res) => {
    try {
      const { id } = req.params;
      ...
      res.status(500).json({ error: "Erro ao buscar formalização" });
    }
  });

  app.put("/api/formalizacao/:id", authMiddleware, async (req: any, res) => {
    try {
      const { id } = req.params;
      ...
      const oldHistoricoCount: number = Array.isArray(oldRecord?.historico_situacao)
        ? oldRecord.historico_situacao.length
        : 0;
      ...
      // 3) Substitui "sistema" pelo nome real somente nas entradas NOVAS
      const historico: any[] = Array.isArray(data?.historico_situacao)
        ? data.historico_situacao
        : [];
      const hasNewEntries = historico.length > oldHistoricoCount;

      if (hasNewEntries && userName !== 'sistema') {
        const patchedHistorico = historico.map((entry: any, idx: number) =>
          idx >= oldHistoricoCount && entry.usuario === 'sistema'
            ? { ...entry, usuario: userName }
            : entry
        );

        // 4) Persiste o histórico corrigido
        const { error: patchError } = await supabase
          .from("formalizacao")
          .update({ historico_situacao: patchedHistorico })
          .eq("id", id);
      ...
      console.error("Erro ao atualizar formalização:", error);
