...
-- ============================================================================
-- Atualiza as funções RPC para sincronizar APENAS emendas dos anos 2024, 2025, 2026
-- Execute este script no Supabase para atualizar as funções de sincronização
-- ============================================================================

-- ============================================================
-- ETAPA 1: UPDATE por numero_convenio (com filtro de anos)
-- ============================================================
...
BEGIN
  WITH matched AS (
    ...
    WHERE TRIM(f.numero_convenio) = TRIM(e.num_convenio)
      ...
      -- 🎯 FILTRO DE ANOS: Apenas 2024, 2025, 2026
      AND e.ano_refer IN ('2024', '2025', '2026')
    RETURNING f.id
  )
  SELECT COUNT(*) INTO v_updated FROM matched;
  RETURN json_build_object('updated', v_updated, 'filtered_years', '2024,2025,2026');
END;
$$;

-- ============================================================
-- ETAPA 2: UPDATE por emenda/codigo_num (com filtro de anos)
-- ============================================================
...
-- ============================================================
-- ETAPA 3: INSERT novas emendas (com filtro de anos)
-- ============================================================
...
BEGIN
  ...
  RETURN json_build_object('inserted', v_inserted, 'filtered_years', '2024,2025,2026');
END;
$$;

-- ============================================================================
-- VERIFICAÇÃO: Contar quantas emendas serão sincronizadas
-- ============================================================================
SELECT 
  'PREVIEW SINCRONIZAÇÃO' as info,
  COUNT(*) as emendas_2024_2025_2026,
  COUNT(DISTINCT ano_refer) as anos_unicos
FROM emendas
...
SELECT 
  ...
  COUNT(DISTINCT codigo_num) as emendas_unicas
FROM emendas
WHERE ano_refer IN ('2024', '2025', '2026')
GROUP BY ano_refer
