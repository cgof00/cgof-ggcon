...
export const onRequest: PagesFunction = async (context) => {
  ...
  if (request.method === 'POST') {
    try {
      ...
      if (role && !['admin', 'usuario', 'visualizador', 'intermediario'].includes(role)) {
        return new Response(JSON.stringify({ error: "Role inválido - deve ser 'admin', 'usuario', 'visualizador' ou 'intermediario'" }), {
          status: 400, headers: { 'Content-Type': 'application/json' }
        });
      }
