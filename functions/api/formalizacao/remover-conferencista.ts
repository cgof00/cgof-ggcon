export const onRequest: PagesFunction = async (context) => {
  ...
  // ── PROTEÇÃO: somente admin pode remover conferencista ───────────────────
  ...
  if (callerDecoded?.role !== 'admin') {
    return new Response(JSON.stringify({ error: 'Acesso negado: somente administradores podem remover o conferencista.' }), {
  ...
  try {
    ...
    // Buscar estado atual antes de remover (para auditoria)
    const beforeResp = await fetch(
      `${SUPABASE_URL}/rest/v1/formalizacao?select=id,demanda,conferencista&id=in.(${idsFilter})`,
    ...
    try {
      ...
      const auditRows = validIds.map((id: any) => {
        const before = beforeMap.get(parseInt(id));
        return {
          formalizacao_id: parseInt(id),
          demanda: before?.demanda || null,
          tecnico_anterior: before?.conferencista || null,
          admin_nome: adminNome,
          admin_role: adminRole,
          acao: 'remover_conferencista',
          valor_anterior: before?.conferencista || null,
        };
      });
