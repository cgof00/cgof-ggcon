export const onRequest: PagesFunction = async (context) => {
  ...
  // Decode admin info from JWT token for audit log
  ...
  // ── PROTEÇÃO: somente admin pode atribuir técnico ────────────────────────
  if (adminRole !== 'admin') {
    return new Response(JSON.stringify({ error: 'Acesso negado: somente administradores podem atribuir o técnico.' }), {
  ...
  try {
    ...
    if (!userResp.ok) {
      return new Response(JSON.stringify({ error: 'Erro ao buscar técnico' }), {
        status: 400, headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    let expandedIds: number[] = [...validIds];
    try {
      ...
      if (demandaResp.ok) {
        ...
        if (demandas.length > 0) {
          // Busca todos os registros que compartilham o mesmo número de demanda
    ...
    // Atualizar formalizações - Supabase REST API usa filter com in
    ...
    try {
      if (adminNome && adminNome !== 'desconhecido') {
        // Minuto atual no fuso SP (UTC-3) — mesmo formato do trigger
        ...
        if (patchPromises.length > 0) {
          console.log(`✅ Histórico atualizado com admin: ${adminNome} (${patchPromises.length} registros)`);
        }
      }
    } catch (_) { /* não deve quebrar a operação principal */ }
    // ──────────────────────────────────────────────────────────────────────────

    // ── Auditoria: registrar quem atribuiu e a quem ──────────
    try {
    ...
    } catch (_) { /* auditoria não deve quebrar a operação principal */ }

    // ── Criar notificação de recebimento para o técnico ───────────────────────
    ...
    try {
      ...
      if (notifResp.ok) {
        notifCriada = true;
        console.log(`✅ Notificação criada para técnico ${tecnico.nome} (id=${usuario_id}, ${expandedIds.length} demandas)`);  
      } else {
    ...
    // ── Sinalizar force-reload para todos os clientes (técnicos, admin) ────
