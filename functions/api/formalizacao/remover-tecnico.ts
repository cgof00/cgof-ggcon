export const onRequest: PagesFunction = async (context) => {
  ...
  // ── PROTEÇÃO: somente admin pode remover técnico ─────────────────────────
  if (adminRole !== 'admin') {
    return new Response(JSON.stringify({ error: 'Acesso negado: somente administradores podem remover o técnico.' }), {
  ...
  try {
    ...
    // Buscar estado atual antes de remover (para o log de auditoria)
    const beforeResp = await fetch(
      `${SUPABASE_URL}/rest/v1/formalizacao?select=id,demanda,tecnico&id=in.(${idsFilter})`,
      {
        headers: {
    ...
    const beforeData: any[] = beforeResp.ok ? await beforeResp.json() : [];
    const beforeMap = new Map(beforeData.map((r: any) => [r.id, r]));
    ...
    // ── Auditoria: registrar quem removeu e de qual técnico ────────────
    try {
      const auditRows = validIds.map((id: any) => {
        const rec = beforeMap.get(parseInt(id));
        return {
          formalizacao_id: parseInt(id),
          demanda: rec?.demanda || null,
          tecnico_anterior: rec?.tecnico || null,
          tecnico_novo: null,
